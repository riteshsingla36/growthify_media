...
const test = inngest.createFunction(
  ...
  { cron: "TZ=Asia/Kolkata 33 23 * * *" }, // The cron syntax for the function. TZ= is optional.

  // This function will be called on the schedule above
  async ({ step }) => {
    const res = await axios.post(`${process.env.BASE_URL}/api/send_slack_message`, {
        "channelName": 'ritesh_kumar_emp_001',
        "message": "This is an automated message send by the scheduler"
    })
    return res.data // You can write whatever you want here.
  }
);
